...
pub async fn tradeable(pool: web::Data<PgPool>) -> Result<HttpResponse> {
    ...
    let tradeable_symbol: Vec<Symbol> = sqlx::query_as::<_, Symbol>(
        "SELECT 
            ...
            FROM symbol WHERE is_margin_enabled = true AND enable_trading = true AND fee_category = 1 AND quote_currency = 'USDT' AND base_currency <> 'USDC' ORDER BY updated_at DESC;",
    )
